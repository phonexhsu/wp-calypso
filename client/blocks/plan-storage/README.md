...
* `onClick`: An on click handler that is fired when the plan block is clicked.
* `className`: A string that adds additional class names to this component.


You can also use PlanStorageBlock directly if you need more control over when
media storage limits are fetched.

#### Usage:

```javascript
import PlanStorageBlock from 'blocks/plan-storage/block';
...
* `mediaStorage`: Media Storage limits for a given site. If this is not provided, the block will not render.
* `onClick`: An on click handler that is fired when the plan storage block is clicked.
* `className`: A string that adds additional class names to this component.
